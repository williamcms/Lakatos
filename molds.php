		...
		<?php
			if(isset($_GET['pagename']) && $stmt->num_rows){
				...
				echo '<article class="product">
						<section class="image">
							'. (!$browser->isMobile() ? '<picture style="background: url('. $row[0]['pack_mold_banner'] .') no-repeat;"></picture>' : 
								'<picture><img src="'. $row[0]['pack_mold_banner'] .'" alt="'. $row[0]['pack_name'] .'" /></picture>') .'
						</section>
				...
				echo '<article class="products">
					<section class="banner">
						<figure class="chapas"><figcaption class="right"><h1>Moldes</h1></figcaption></figure>
					</section>
					...
					<section class="list">
						...
						for($i = 0; $i < $stmt->num_rows; $i++){
							echo '<figure>
								<a href="'. url() .'/molde/'. $row[$i]['pack_pagename'] .'" class="d-flex">
								<img src="'. $row[$i]['pack_mold'] .'" alt="" width="" height="" />
								<figcaption>
