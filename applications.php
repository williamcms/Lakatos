<?php 
	...
	if(isset($_GET['pagename'])){
		...
		if($stmt = $conn->link->prepare("SELECT * FROM packaging WHERE pack_name = ? LIMIT 1")){
			try{
...
<html lang="pt-BR">
<head>
	<title><?php echo $website->title .': '. (isset($pagename) ? $row[0]['pack_name'] : 'Embalagens') ?></title>
		...
		<?php
			if(isset($_GET['pagename']) && $stmt->num_rows){
				...
				echo '<article class="product">
						<section class="image">
							'. (!$browser->isMobile() ? '<picture style="background: url('. $row[0]['pack_image'] .') no-repeat;"></picture>' : 
								'<picture><img src="'. $row[0]['pack_image'] .'" alt="'. $row[0]['pack_name'] .'" /></picture>') .'
						</section>

						<section class="info">
							<div class="title pt-3 pl-3 pr-4">
								<h1>'. $row[0]['pack_name'] .'</h1>
							</div>
				...
				echo '<article class="products">
					<section class="banner">
						<figure class="chapas"><figcaption class="right"><h1>Embalagens <br/>para alimentos</h1></figcaption></figure>
					</section>
					...
					<section class="list">
						...
						for($i = 0; $i < $stmt->num_rows; $i++){
							echo '<figure>
								<a href="'. url() .'/aplicacao/'. $row[$i]['pack_name'] .'">
								<img src="'. $row[$i]['pack_image'] .'" alt="" width="" height="" />
								<figcaption><h3>'. $row[$i]['pack_goal'] .'</h3></figcaption></a>
							</figure>';
						}
					echo '</div>
					</section>
